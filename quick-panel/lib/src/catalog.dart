...
/// display order.
class TileSpec {
  const TileSpec.toggle(this.toggleId, {required this.icon, this.large = false})
    : kind = TileKind.toggle,
      actionId = null;

  const TileSpec.action(this.actionId, {required this.icon, this.large = false})
...
const panelTiles = <TileSpec>[
  ...
  TileSpec.toggle(ToggleId.boundary, icon: Icons.crop_free, large: true),
  TileSpec.toggle(ToggleId.bluetooth, icon: Icons.bluetooth, large: true),
  TileSpec.toggle(ToggleId.seethrough, icon: Icons.visibility, large: true),
  TileSpec.toggle(ToggleId.microphone, icon: Icons.mic),
  TileSpec.action(ActionId.resetView, icon: Icons.center_focus_strong),
  ...
  TileSpec.action(ActionId.reportProblem, icon: Icons.flag_outlined),
