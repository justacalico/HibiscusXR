...
LOG=/mnt/f/PN2Lineage/notes/111_syms.txt
exec >"$LOG" 2>&1

echo "=== in what we already extracted ==="
for d in /mnt/f/PN2Lineage/pvr_stack /mnt/f/PN2Lineage/pvr_applibs; do
...
for f in /mnt/f/PN2Lineage/pvr_stack/lib64/libpxr_6dof_optimization.so \
         /mnt/f/PN2Lineage/pvr_stack/lib/libpxr_6dof_optimization.so; do
  [ -f "$f" ] || continue
  echo "--- $f ---"
