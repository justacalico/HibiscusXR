# Clean post-reboot baseline: everything mapping the patched library, fancontrol
# started by init rather than by hand, controller service enabled from boot.
param([string]$Dev = '192.168.0.172:5555')
$adb = 'C:\adb\adb.exe'
$log = 'F:\PN2Lineage\notes\374_postboot.log'
function L($m) { $s = "[{0}] {1}" -f (Get-Date -f 'HH:mm:ss'), $m; Add-Content $log $s; Write-Host $s }
Set-Content $log "=== post-reboot baseline $(Get-Date) ==="
...
[System.IO.File]::WriteAllText('F:\PN2Lineage\tools\_pb.sh', $sh + "`n")
& $adb -s $Dev push F:\PN2Lineage\tools\_pb.sh /data/local/tmp/ 2>&1 | Out-Null
(& $adb -s $Dev shell "su -c 'sh /data/local/tmp/_pb.sh'" 2>&1) | ForEach-Object { L ("   " + ($_ -replace '^\d\d-\d\d \S+\s+','')) }
& $adb -s $Dev shell "screencap -p /sdcard/pn2_pb.png" 2>&1 | Out-Null
& $adb -s $Dev pull /sdcard/pn2_pb.png F:\PN2Lineage\notes\pn2_pb.png 2>&1 | Out-Null
