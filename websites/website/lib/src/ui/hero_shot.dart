...
/// The hero image: the view inside the HMD - a vrhome window floating in
/// the headset's dark environment, a second panel behind it, a faint
/// floor grid and a lens vignette. All drawn in widgets so it stays
/// sharp at any size. Always dark: it depicts the headset, not the theme.
              ...
              child: Stack(
                children: [
                  ...
                  Positioned(
                    ...
                    child: Opacity(
                      opacity: 0.4,
                      child: _Panel(width: 480),
                    ),
                  ),
                  const Positioned(
                    left: 158,
                    top: 84,
                    child: _Panel(width: 620),
                  ),
                  const Positioned.fill(
...
/// The space around the windows: near-black depth, a blue glow behind
/// the main panel, a perspective floor grid and a few dust motes.
class _StagePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    canvas.drawRect(
      ...
      Paint()
        ..shader = const LinearGradient(
          ...
          colors: [AppColors.shotSurface, AppColors.shotSpace],
        ).createShader(const Rect.fromLTWH(0, 0, 880, 550)),
    );

    canvas.drawCircle(
      const Offset(470, 290),
      380,
      Paint()
        ..shader = RadialGradient(
          colors: [
            AppColors.accentOnDark.withValues(alpha: 0.13),
            AppColors.accentOnDark.withValues(alpha: 0),
          ],
        ).createShader(
            Rect.fromCircle(center: const Offset(470, 290), radius: 380)),
    );
    ...
    for (var i = 1; i <= 8; i++) {
      final t = i / 8;
      final y = horizon + t * t * 200;
      canvas.drawLine(
        Offset(0, y),
        Offset(880, y),
        grid..color = AppColors.accentOnDark.withValues(alpha: 0.04 + 0.08 * t),
      );
    }
    for (var i = -8; i <= 8; i++) {
      canvas.drawLine(
        const Offset(440, horizon),
        Offset(440 + i * 130, 550),
        grid..color = AppColors.accentOnDark.withValues(alpha: 0.06),
      );
    }

    final mote = Paint()..isAntiAlias = true;
    for (var i = 0; i < 18; i++) {
      final x = (i * 167 + 43) % 880.0;
      final y = 40.0 + (i * 89 + 17) % 250;
      mote.color =
          AppColors.shotDisc.withValues(alpha: 0.05 + (i % 3) * 0.05);
      canvas.drawCircle(Offset(x, y), 1.1 + (i % 3) * 0.5, mote);
    }
  }
...
/// Lens falloff drawn over everything, like the binocular mask of an
/// actual headset view.
    ...
    canvas.drawRect(
      ...
      Paint()
        ..shader = RadialGradient(
          radius: 1.15,
          colors: [
            AppColors.darkPaper.withValues(alpha: 0),
            AppColors.darkPaper.withValues(alpha: 0.62),
          ],
          stops: const [0.62, 1],
        ).createShader(const Rect.fromLTWH(0, 0, 880, 550)),
    );
...
/// One floating app window - a rounded dark panel with a soft shadow,
/// showing the library UI inside.
class _Panel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      width: width,
      height: width * 550 / 880,
      decoration: BoxDecoration(
        color: AppColors.shotSurface,
        borderRadius: BorderRadius.circular(20),
        border:
            Border.all(color: AppColors.shotDisc.withValues(alpha: 0.09)),
        boxShadow: [
          BoxShadow(
            color: AppColors.darkPaper.withValues(alpha: 0.6),
            blurRadius: 60,
...
/// The library window contents: status bar, toolbar, then the app grid
/// clipped partway through the third row like the real screenshot.
class _LibraryWindow extends StatelessWidget {
  const _LibraryWindow();
                ...
                child: Column(
                  children: [
                    for (var row = 0; row < 3; row++)
                      Padding(
                        padding:
                            EdgeInsets.only(bottom: row == 2 ? 0 : 22),
                        child: Row(
                          children: [
...
class LibraryMarkPainter extends CustomPainter {
  ...
  void _calendar(Canvas canvas, Offset c) {
    ...
    canvas.drawRRect(rrect,
        _stroke..color = AppColors.shotMuted..strokeWidth = 1);
    canvas.drawRRect(
    ...
    tp.paint(
      canvas,
      Offset(c.dx - tp.width / 2,
          page.top + 10 + (page.height - 10 - tp.height) / 2),
    );
  }
  ...
  void _camera(Canvas canvas, Offset c) {
    ...
    canvas.drawCircle(
        c + const Offset(0, 2), 8, _fill..color = AppColors.shotPaper);
    canvas.drawCircle(
        c + const Offset(0, 2), 4, _fill..color = AppColors.shotGoogle);
  ...
  void _strip(
      Canvas canvas, Offset a, Offset b, Offset inside, Color color) {
    const t = 8.0;
  ...
  void _files(Canvas canvas, Offset c) {
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromCenter(
            center: c + const Offset(0, 6), width: 19, height: 15),
        const Radius.circular(3),
      ),
      _fill..color = AppColors.shotPaper,
    );
  ...
  void _glasses(Canvas canvas, Offset c) {
    ...
    for (final dx in [-11.5, 11.5]) {
      canvas.drawRRect(
        RRect.fromRectAndRadius(
          Rect.fromCenter(
              center: c + Offset(dx, 3), width: 16, height: 11),
